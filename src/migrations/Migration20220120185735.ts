...
export class Migration20220120185735 extends Migration {
	async up(): Promise<void> {
		...
		this.addSql(
			'create table "favourite" ("id" serial primary key, "idx" varchar(255) not null default uuid_generate_v4(), "is_active" boolean not null default true, "is_obsolete" boolean not null default false, "deleted_at" timestamptz(0) null, "created_at" timestamptz(0) not null default CURRENT_TIMESTAMP, "updated_at" timestamptz(0) null default CURRENT_TIMESTAMP, "post_id" int not null, "user_id" int not null);',
		);
		...
		this.addSql(
			'alter table "favourite" add constraint "favourite_post_id_foreign" foreign key ("post_id") references "post" ("id") on update cascade;',
		);
		this.addSql(
			'alter table "favourite" add constraint "favourite_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade;',
		);
	...
	async down(): Promise<void> {
		...
		this.addSql(
			'alter table "favourite" drop constraint "favourite_user_id_foreign";',
		);
		...
		this.addSql(
			'alter table "favourite" drop constraint "favourite_post_id_foreign";',
		);
		...
		this.addSql('drop table if exists "favourite" cascade;');
