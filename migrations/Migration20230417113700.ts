...
export class Migration20230417113700 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table `conversation` (`id` integer not null primary key autoincrement, `idx` text not null, `is_active` integer not null default true, `is_obsolete` integer not null default false, `deleted_at` datetime null, `created_at` datetime not null, `updated_at` datetime null, `chat_name` text not null);');
    ...
    this.addSql('create table `message` (`id` integer not null primary key autoincrement, `idx` text not null, `is_active` integer not null default true, `is_obsolete` integer not null default false, `deleted_at` datetime null, `created_at` datetime not null, `updated_at` datetime null, `body` text not null, `sender_id` integer not null, `conversation_id` integer not null, `is_read` integer not null default false, `read_at` datetime null, constraint `message_sender_id_foreign` foreign key(`sender_id`) references `user`(`id`) on update cascade, constraint `message_conversation_id_foreign` foreign key(`conversation_id`) references `conversation`(`id`) on update cascade);');
